...
export default function ProjectCard({ project, onOpen }) {
  return (
    <motion.div
      ...
      className="glass-card rounded-2xl p-5 flex flex-col justify-between transition hover:shadow-[0_25px_60px_-30px_rgba(34,211,238,0.35)]"
    >
      <div>
        <h3 className="text-lg font-bold tracking-tight">{project.title}</h3>
        <p className="mt-1.5 text-white/70 text-[13px]">{project.subtitle}</p>
        ...
        <ul className="mt-3.5 space-y-1 text-white/80 text-[13px]">
          ...
          {project.features.length > 3 && (
            <li className="text-white/60 text-[11px]">+ more in details</li>
          )}
        </ul>
      </div>
